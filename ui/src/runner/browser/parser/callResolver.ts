...
function resolveSingleCall(
  ...
  if (ref.kind === "attribute" && ref.receiver && classRegistry.has(ref.receiver)) {
    const cls = classRegistry.get(ref.receiver)!;
    // Check class children
  ...
  if (ref.kind === "bare" && classRegistry.has(ref.name)) {
    const cls = classRegistry.get(ref.name)!;
...
function findEnclosingClass(
  ...
  for (let i = parts.length - 1; i >= 1; i--) {
    ...
    if (classRegistry.has(potentialClassName)) {
      const cls = classRegistry.get(potentialClassName)!;
