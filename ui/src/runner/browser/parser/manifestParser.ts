...
export function parseManifest(path: string, content: string): ManifestParseResult {
  ...
  switch (basename) {
    ...
    default:
      return { dependencies: [], errors: [`Unsupported manifest: ${basename}`] };
  }
}
...
export function parsePackageJson(content: string, source: string): ManifestParseResult {
  const errors: string[] = [];
...
export function parseGoMod(content: string, source: string): ManifestParseResult {
  const deps: ParsedDependency[] = [];
  const errors: string[] = [];
...
function parseGoRequireLine(line: string, source: string): ParsedDependency | null {
...
export function parseRequirementsTxt(content: string, source: string): ManifestParseResult {
  const deps: ParsedDependency[] = [];
  const errors: string[] = [];
 ...
 */
export function parsePyprojectToml(content: string, source: string): ManifestParseResult {
  const deps: ParsedDependency[] = [];
  const errors: string[] = [];
  ...
  while (i < lines.length) {
    ...
    if (currentSection === "[tool.poetry.dependencies]") {
      ...
      if (eqIdx > 0 && !line.startsWith("#") && !line.startsWith("[")) {
        ...
        if (name === "python") { i++; continue; } // skip python version constraint
...
function collectTomlArray(lines: string[], startLine: number, charOffset: number): string {
...
function parsePep508(spec: string, source: string, depType = "runtime"): ParsedDependency | null {
