...
`);
      const result = extractPython(root);
      const calls = result.symbols[0].calls;
      const bareNames = calls
        .filter((c) => c.kind === 'bare')
        .map((c) => c.name);
...
`);
      const result = extractPython(root);
      const runMethod = result.symbols[0].children.find(
        (c) => c.name === 'run',
      )!;
      expect(runMethod.calls).toContainEqual({
        name: 'validate',
        receiver: 'self',
        kind: 'attribute',
      });
...
`);
    ...
    it('extracts multiple base classes', async () => {
      const root = await parsePy(`class Admin(User, Mixin):
    pass
`);
      const result = extractPython(root);
      expect(result.symbols[0].superclasses).toEqual(['User', 'Mixin']);
    });

    it('no superclasses for plain class', async () => {
      const root = await parsePy(`class Foo:
    pass
`);
      const result = extractPython(root);
    ...
    it('extracts dotted base class name', async () => {
      const root = await parsePy(`class MyView(base.View):
    pass
`);
      const result = extractPython(root);
      expect(result.symbols[0].superclasses).toEqual(['base.View']);
    });
  });
