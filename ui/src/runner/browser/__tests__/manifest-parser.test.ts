...
`;
  ...
  it("parses block and single-line requires", () => {
    ...
    const mux = result.dependencies.find(d => d.name === "github.com/gorilla/mux")!;
    expect(mux.version).toBe("v1.8.1");
    expect(mux.registry).toBe("go");
    expect(mux.dependencyType).toBe("runtime");

    const text = result.dependencies.find(d => d.name === "golang.org/x/text")!;
...
describe("packageId", () => {
  it("produces deterministic IDs", () => {
    ...
    expect(packageId("go", "github.com/gorilla/mux")).toBe("pkg:go:github.com/gorilla/mux");
