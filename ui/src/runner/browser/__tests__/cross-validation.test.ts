...
const FIXTURE_DIR = join(__dirname, "..", "..", "..", "..", "..", "tests", "fixtures", "cross-validation");
...
describe("cross-validation: TS extractor matches expected", () => {
  for (const fixtureName of fixtures) {
    it(fixtureName, async () => {
      const source = readFileSync(join(FIXTURE_DIR, `${fixtureName}.txt`), "utf-8");
      const expected = JSON.parse(
        readFileSync(join(FIXTURE_DIR, `${fixtureName}.expected.json`), "utf-8"),
      );
