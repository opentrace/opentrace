...
export interface IndexingCallbacks {
  onProgress: (phase: IndexPhase, message: string, detail: ProgressDetail) => void;
...
export class IndexingManager {
  ...
  private parseResult: { filesProcessed: number; errors: string[] } = { filesProcessed: 0, errors: [] };
  private startTime = 0;
  ...
  async start(input: LoaderInput) {
    ...
    if (this.enrichmentEnabled) {
      ...
      this.enrichWorker.onmessage = async (e: MessageEvent<EnrichWorkerResponse>) => {
    ...
    this.callbacks.onProgress("initializing", "Loading parsers...", { current: 0, total: 1 });
    this.postToParseWorker({ type: "init", summarizerConfig });

    // 4. Fetch repo in parallel with worker init
    const fetchingMessage = input.kind === "directory" ? "Loading files..." : "Fetching repository tree...";
    this.callbacks.onProgress("fetching", fetchingMessage, { current: 0, total: 1 });

    const progressCallback = (progress: FetchProgress) => {
      if (progress.phase === "tree") {
        const treeMessage = input.kind === "directory" ? "Scanning directory..." : "Downloading repository tree...";
        this.callbacks.onProgress("fetching", treeMessage, {
          current: progress.current,
          total: progress.total,
        });
      } else {
        this.callbacks.onProgress("fetching", `Loading ${progress.fileName ?? "files"}...`, {
          current: progress.current,
          total: progress.total,
    ...
    this.callbacks.onProgress("fetching", `Fetched ${repo.files.length} files`, {
      current: repo.files.length,
      total: repo.files.length,
    });
    this.callbacks.onStageComplete("fetching", `Fetched ${repo.files.length} files`);
    ...
    if (this.store) {
      ...
      this.store.storeSource(
        repo.files.map(f => ({ id: `${repoId}/${f.path}`, path: f.path, content: f.content })),
      );
    }

    // Send repo to parse worker if ready, otherwise buffer
    if (this.parseWorkerReady) {
      this.callbacks.onProgress("parsing", "Starting extraction...", { current: 0, total: 1 });
    ...
    switch (msg.type) {
      ...
      case "stage-complete":
        ...
        if (msg.phase === "resolving" && !this.persistedSignaled) {
          ...
          this.callbacks.onStageComplete("submitting", `Submitted ${this.totalNodesSubmitted} nodes, ${this.totalRelsSubmitted} relationships`);
          this.callbacks.onPersisted?.(this.totalNodesSubmitted, this.totalRelsSubmitted);
        }
        break;

      case "batch": {
        const p = this.submitBatch({ nodes: msg.nodes, relationships: msg.relationships });
      ...
      case "done":
        ...
        this.parseResult = { filesProcessed: msg.filesProcessed, errors: msg.errors };
        this.parseDone = true;
  ...
  private flushEnrichmentBuffer() {
    if (!this.enrichmentReady || this.enrichmentBuffer.length === 0 || !this.enrichWorker) return;
  ...
  private async fetchRepo(
    ...
    if (!loader) {
      throw new Error("No loader found. Provide a GitHub URL, GitLab URL, or local directory.");
    }
