...
For wiki pages: agent / model / session / confidence stamped at compile time
plus the CITES chain back through any source-summary pages to the original
``Source`` artefacts (sha256 + filename, no retained bytes).

For code nodes: ``commit_sha`` and ``indexer_version`` are read from the
per-repo ``IndexMetadata`` node written by ``opentraceai index``; file path
and line range come from the node itself.
"""
...
def provenance(store: GraphStore, node_id: str) -> dict[str, Any]:
    ...
    -------
    dict
        ``{node_id, node_type, kind, code: …|null, wiki: …|null}`` where
        ``kind`` is ``"code"``, ``"wiki"``, or ``"unknown"``. The unused
        sub-payload is ``null``.
    """
...
def _wiki_provenance(store: GraphStore, node_id: str, node_type: str, props: dict[str, Any]) -> dict[str, Any]:
    ...
    concept → source-summary page → Source. For a source-summary page it
    goes source-summary → Source. For a Source, the chain is just itself.
    """
