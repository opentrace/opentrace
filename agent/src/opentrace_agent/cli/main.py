...
def benchmark(
    ...
    if codebase and tasks_path:
        # Index + benchmark in one shot
        report = index_and_benchmark(codebase, tasks_path, repo_id=repo_id, db_path=db_path)
        _print_report(report, output_format, json_mod)
        return
...
def _print_report(report: object, output_format: str, json_mod: object) -> None:
    if output_format == "json":
        click.echo(json_mod.dumps(report.to_dict(), indent=2))  # type: ignore[union-attr]
    else:
        click.echo(report.summary())  # type: ignore[union-attr]
