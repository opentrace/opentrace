...
# Default wall-clock budget for a `repo_index` subprocess, in seconds.
# Matches the opencode plugin's `indexTimeout` (30 min) so large monorepos
# that index fine there don't trip a 10-min ceiling here.
_DEFAULT_INDEX_TIMEOUT = 1800


def _resolve_index_timeout() -> int | None:
    """Resolve the `repo_index` subprocess timeout from the environment.

    ``OPENTRACE_INDEX_TIMEOUT`` (seconds) overrides the default. ``0`` means
    no timeout (returns ``None``, matching opencode's ``<= 0`` semantics).
    Unparseable values fall back to the default rather than failing the call.
    """
    raw = os.environ.get("OPENTRACE_INDEX_TIMEOUT")
    if raw is None or not raw.strip():
        return _DEFAULT_INDEX_TIMEOUT
    try:
        val = int(raw.strip())
    except ValueError:
        return _DEFAULT_INDEX_TIMEOUT
...
def create_mcp_server(
    ...
    *db_path* is used by ``repo_index`` to direct the indexer at the same
    ...
    def keyword_search(query: str, nodeTypes: str = "", limit: int = 10) -> str:
        """Search the graph by keyword(s) or a description of what code
        does — works with multi-word phrases.

        This is the "find code by meaning" entry point: it matches over
        names, signatures, docs, AND LLM-generated summaries, so a query
        like "functions that validate user input" can surface a symbol
        named ``sanitize`` whose summary mentions validation.
        ...
        handles auth tokens") or describes behaviour rather than a name;
        use ``search_graph`` when the user gives a single literal symbol
        name.
        ...
        Caveat: this ranks by keyword *coverage*, not whole-phrase
        relevance, and it is not vector/embedding search (that's planned).
        For score-ranked, stemmed, whole-phrase matching use ``fts_search``
        instead.  Run ``opentraceai augment`` to enrich nodes with
        LLM-generated summaries — they're folded into ``search_text`` and
        improve recall for both tools on natural-language queries.
        """
        if not store:
    ...
    # -------------------------------------------------------------------
    # fts_search — whole-phrase full-text ranking (BM25 / Porter stemmer).
    # -------------------------------------------------------------------
    @server.tool()
    def fts_search(query: str, nodeTypes: str = "", repo: str = "", limit: int = 10) -> str:
        """Full-text search ranked by relevance across the whole query phrase.

        Passes the *entire* query to the graph's FTS index (BM25 scoring
        with a Porter stemmer) over each node's ``search_text`` — which
        folds in name, signature, summary, path, and docs. Results come
        back ordered by relevance score, highest first.

        How this differs from ``keyword_search``: ``keyword_search``
        splits the query into keywords, searches each separately, and
        ranks by *how many* keywords a node hit — good when the user lists
        several distinct concepts. ``fts_search`` ranks the *whole phrase*
        as one unit with stemming (so "validating users" matches "validate
        user"), which is better for a single natural-language description
        or when you want score-ordered relevance rather than keyword
        coverage. When unsure, ``keyword_search`` is the safer default;
        reach for ``fts_search`` when you want phrase-level relevance
        ranking or the keyword merge is returning noise.

        Args:
            query: A phrase or description of the code you're looking for.
            nodeTypes: Optional comma-separated filter (e.g. "Function,Class").
            repo: Optional repo id to scope the search to one repository.
            limit: Max results (default 10, clamped to [1, 1000]).

        Each result carries its FTS ``score`` plus the node's id, name,
        type, and properties. Read the source via ``source_read`` with a
        result's id to confirm a match.
        """
        if not store:
            logger.info("fts_search called but no index exists")
            return NO_INDEX_MSG
        logger.debug("fts_search(query=%r, limit=%d, nodeTypes=%r, repo=%r)", query, limit, nodeTypes, repo)
        try:
            types = [t.strip() for t in nodeTypes.split(",") if t.strip()] or None
            limit = max(1, min(limit, 1000))
            ...
            return _error_response("fts_search", e)
    ...
    def source_read(
        ...
        A line-sliced read is returned ``cat -n``-style with a header and
        line numbers; a whole-file read returns the raw file contents.
        """
        if not store:
            return NO_INDEX_MSG
        logger.debug(
    ...
    def repo_index(path_or_url: str, repoId: str = "", ref: str = "") -> str:
        ...
        - **Git URL** (``https://``, ``http://`` or ``git@``): shells out
          ...
          Private repos work when a token is available: set
          ``OPENTRACE_GIT_TOKEN`` / ``GITHUB_TOKEN`` / ``GITLAB_TOKEN`` in
          the server's environment, or store one with ``opentraceai auth
          git`` (the subprocess resolves it automatically — no token is
          passed through this tool call).

        The subprocess timeout defaults to 30 minutes; override with the
        ``OPENTRACE_INDEX_TIMEOUT`` env var (seconds, ``0`` = no limit).
        ...
        timeout = _resolve_index_timeout()
        try:
            ...
            return json.dumps(
                {
                    "error": (
                        f"Indexing timed out after {mins:.0f} minutes (set OPENTRACE_INDEX_TIMEOUT to raise the limit)"
                    )
                }
            )
