...
@app.command("llm-extraction-eval")
@click.option(
    "--corpus",
    "corpus_dir",
    default=None,
    type=click.Path(exists=True, file_okay=False),
    help="Corpus directory (defaults to the bundled benchmarks/corpus).",
)
@click.option(
    "--output",
    "output_format",
...
@click.option("-v", "--verbose", is_flag=True, help="Per-example breakdown.")
def llm_extraction_eval(corpus_dir: str | None, output_format: str, verbose: bool) -> None:
    """Score an LLM extractor against the hand-labelled corpus.

    With no extractor wired up, this reports a baseline of 0% precision and
    is useful for verifying the harness itself or measuring corpus growth.
    """
    ...
    if corpus_dir is None:
        corpus_dir = str(Path(__file__).resolve().parents[1] / "benchmarks" / "corpus")

    benchmark = LLMExtractionEvalBenchmark(extractor=null_extractor)
    report = benchmark.run(corpus_dir)
    _print_report(report, output_format, _json, verbose=verbose)
