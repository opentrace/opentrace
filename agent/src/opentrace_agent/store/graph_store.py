...
# When an FTS query is scoped to a single repo, the ``QUERY_FTS_INDEX``
# ranking still runs globally, so the top-N hits may all live in other
# repos and get filtered out. Over-fetch before the repo predicate so a
# scoped search still has enough candidates to fill ``limit``.
...
class GraphStore:
    ...
    def fts_search(
        ...
        *,
        ...
        """Whole-phrase full-text search over ``search_text``.

        ``search_text`` folds in name, signature, summary, path, and docs,
        so this ranks nodes by how well the *entire* query phrase matches
        — Porter-stemmed and BM25-scored by Kuzu's ``QUERY_FTS_INDEX``.

        This differs from :meth:`search_nodes` (which the keyword tools
        call once per tokenized keyword and merge): here the full query
        string is passed through in a single round trip, and optional
        ``node_types`` / ``repo_id`` filters are pushed into the same
        Cypher so ranking and filtering happen together.

        Returns nodes as ``{id, name, type, properties, score}`` ordered
        by score descending. When the FTS extension is unavailable the
        call falls back to :meth:`search_nodes` (substring) with a
        ``score`` of ``0.0`` so the result shape stays stable.
        """
        limit = max(1, limit)
        # Over-fetch only when a repo filter is present (see constants).
        ...
        try:
            ...
            logger.debug("FTS search failed, using substring fallback", exc_info=True)
            return [{**n, "score": 0.0} for n in self.search_nodes(query, node_types=node_types, limit=limit)]
