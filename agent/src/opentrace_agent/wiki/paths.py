...
# Lock files and per-compile audit logs are noise; the vault metadata can be
...
def ensure_vault_layout(
    ...
    *,
    ...
    """Create the vault directory layout (.compile-log/) if missing.

    A vault dir holds ``.vault.json`` and ``.compile-log/`` only — document
    bodies live in the shared, sha-keyed corpus dir, not under the vault.
    """
    root_path = vault_root(root, scope=scope, project_root=project_root)
