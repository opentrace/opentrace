...
def seeded_store(store):
    ...
    Wiki:
      Vault(kb) -CONTAINS-> WikiPage(source-summary-spec)
      Vault(kb) -CONTAINS-> WikiPage(auth-flow)
      WikiPage(auth-flow) -CITES-> WikiPage(source-summary-spec)
      WikiPage(source-summary-spec) -CITES-> Source(spec.pdf)
      WikiPage(auth-flow) -LINKS_TO-> WikiPage(source-summary-spec)
    ...
    meta.pages = {
        "source-summary-spec": PageMeta(
            slug="source-summary-spec",
            title="spec.pdf",
    ...
    bodies = {
        "source-summary-spec": "Summary of the OAuth spec.",
        "auth-flow": ("Auth flow consumes [[spec.pdf]] to verify tokens."),
    }
    write_vault_to_graph(
        store,
        meta,
        ...
        compiled_slugs={"source-summary-spec", "auth-flow"},
    )
