...
# ---------------------------------------------------------------------------
# MCP tool: fts_search (whole-phrase FTS ranking)
# ---------------------------------------------------------------------------


class TestMCPFtsSearch:
    def test_returns_scored_results(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="main")
        assert isinstance(result, list)
        assert len(result) > 0
        for node in result:
            assert "id" in node
            assert "type" in node
            assert "name" in node
            # fts_search surfaces the raw FTS relevance score.
            assert isinstance(node["score"], (int, float))

    def test_results_ordered_by_score_desc(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="main", limit=10)
        scores = [n["score"] for n in result]
        assert scores == sorted(scores, reverse=True)

    def test_type_filter(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="main", nodeTypes="File")
        assert isinstance(result, list)
        for node in result:
            assert node["type"] == "File"

    def test_repo_filter_scopes_results(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="main", repo="test/go-project")
        for node in result:
            assert node["id"].startswith("test/go-project/")

    def test_no_results(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="zzz_nonexistent_zzz")
        assert isinstance(result, list)
        assert len(result) == 0

    def test_respects_limit(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="e", limit=2)
        assert len(result) <= 2

    def test_properties_are_valid(self, indexed_go_store):
        result = _call_tool(indexed_go_store, "fts_search", query="db")
        for node in result:
            props = node.get("properties")
            if props is not None:
...
class TestMCPFindUsages:
    ...
    def test_prefers_name_match_over_docs_match(self):
        ...
        assert result["target"]["id"] == target["id"], (
            f"find_usages picked the docstring-mention over the actual symbol — got {result['target']['name']!r}"
        )
...
class TestMCPImpactAnalysis:
    def test_finds_file(self, indexed_python_store):
        ...
        assert "Database" in symbol_names, f"Expected 'Database' in symbols, got {symbol_names!r}"
    ...
    def test_surfaces_file_level_importers(self, indexed_python_store):
        ...
        importer_paths = {(n.get("properties") or {}).get("path", "") for n in result["file_importers"]}
