...
class TestExtractSearchPattern:
    ...
    def test_extracts_grep_after_redirect(self):
        ...
        assert _common.extract_search_pattern("grep -rn findme src/ 2>/dev/null | head") == "findme"
...
class TestContextGuardrails:
    def test_auto_context_defaults_to_opt_in(self):
        assert _common.AUTO_CONTEXT_DEFAULT is False
        assert _common.BASH_AUGMENT_DEFAULT is False

    def test_env_flag_enabled_defaults_true(self, monkeypatch):
        monkeypatch.delenv("OPENTRACE_CLAUDE_AUTO_CONTEXT", raising=False)
        assert _common.env_flag_enabled("OPENTRACE_CLAUDE_AUTO_CONTEXT") is True

    def test_env_flag_enabled_accepts_off_values(self, monkeypatch):
        monkeypatch.setenv("OPENTRACE_CLAUDE_AUTO_CONTEXT", "0")
        assert _common.env_flag_enabled("OPENTRACE_CLAUDE_AUTO_CONTEXT") is False

    def test_cap_context_truncates_long_payload(self):
        capped = _common.cap_context("x" * 100, max_chars=40)
        assert len(capped) <= 40
        assert "truncated" in capped

    def test_context_cache_marks_recent(self, tmp_path, monkeypatch):
        monkeypatch.setattr(_common, "CONTEXT_CACHE_PATH", tmp_path / "context.json")
        monkeypatch.setattr(_common, "CACHE_DIR", tmp_path)
        key = _common.context_cache_key(tmp_path, "PreToolUse", "search", "GraphStore")
        assert _common.context_recently_emitted(key) is False
        _common.mark_context_emitted(key)
        assert _common.context_recently_emitted(key) is True
...
class TestPreToolUseMain:
    ...
    def enable_auto_context(self, monkeypatch):
        monkeypatch.setenv("OPENTRACE_CLAUDE_AUTO_CONTEXT", "1")
        monkeypatch.setenv("OPENTRACE_CLAUDE_AUGMENT_BASH", "1")
    ...
    def test_skips_when_auto_context_disabled(self, healthy_workspace, monkeypatch, capsys):
        monkeypatch.setenv("OPENTRACE_CLAUDE_AUTO_CONTEXT", "0")
        with patch.object(_pre, "build_search_message") as mock_search:
            data = _run_hook(
                _pre,
    ...
    def test_skips_bash_when_bash_augmentation_disabled(self, healthy_workspace, monkeypatch, capsys):
        monkeypatch.setenv("OPENTRACE_CLAUDE_AUGMENT_BASH", "0")
        with patch.object(_pre, "build_search_message") as mock_search:
            data = _run_hook(
                _pre,
                {
    ...
    def test_duplicate_search_suppressed_before_cli(self, healthy_workspace, monkeypatch, capsys, tmp_path):
        monkeypatch.setattr(_common, "CONTEXT_CACHE_PATH", tmp_path / "context.json")
        monkeypatch.setattr(_common, "CACHE_DIR", tmp_path)
        payload = {
            "tool_name": "Grep",
        ...
        with patch.object(_pre, "build_search_message", return_value="[OpenTrace] ctx") as mock_search:
            first = _run_hook(_pre, payload, monkeypatch, capsys)
            second = _run_hook(_pre, payload, monkeypatch, capsys)
...
class TestPostToolUseMain:
    ...
    def test_duplicate_write_impact_suppressed_before_cli(self, healthy_workspace, monkeypatch, capsys, tmp_path):
        monkeypatch.setattr(_common, "CONTEXT_CACHE_PATH", tmp_path / "context.json")
        monkeypatch.setattr(_common, "CACHE_DIR", tmp_path)
        payload = {
            "tool_name": "Write",
        ...
        with patch.object(_post, "run_opentraceai", return_value="[OpenTrace] impact") as mock_run:
            first = _run_hook(_post, payload, monkeypatch, capsys)
            second = _run_hook(_post, payload, monkeypatch, capsys)
        assert first is not None
        assert second is None
        assert mock_run.call_count == 1

    def test_skips_post_hook_when_auto_context_disabled(self, healthy_workspace, monkeypatch, capsys):
        monkeypatch.setenv("OPENTRACE_CLAUDE_AUTO_CONTEXT", "0")
...
class TestSessionStartMain:
    def test_emits_install_guidance_when_cli_missing(self, healthy_workspace, monkeypatch, capsys):
    ...
    def test_starts_background_index_when_no_db(self, tmp_path, monkeypatch, capsys):
    ...
    def test_emits_directive_when_healthy(self, healthy_workspace, monkeypatch, capsys):
        ...
        with (
            patch.object(_session_start, "_installed_version", return_value="0.4.0"),
            patch.object(_session_start, "run_opentraceai", return_value=fake_stats),
    ...
    def test_appends_update_notice_when_available(self, healthy_workspace, monkeypatch, capsys):
        ...
        with (
            patch.object(_session_start, "_installed_version", return_value="0.4.0"),
            patch.object(_session_start, "run_opentraceai", return_value="1 node"),
            patch.object(
                _session_start,
                "_update_notice",
...
class TestUserPromptSubmitMain:
    ...
    def test_skips_when_no_stale_files(self, healthy_workspace, monkeypatch, capsys):
        """A healthy workspace with nothing stale stays silent — the hook
        only speaks up when tracked edits postdate the index, and never
        even consults the throttle when there's nothing to warn about.
        """
        with (
            patch.object(_user_prompt_submit, "stale_files", return_value=[]),
            patch.object(_user_prompt_submit, "briefing_due") as mock_due,
        ):
            data = _run_hook(
                _user_prompt_submit,
                {"cwd": str(healthy_workspace)},
    ...
    def test_skips_when_throttled(self, healthy_workspace, monkeypatch, capsys):
        """The 10-min TTL must throttle re-warning — stale files present but
        a warning already sent in this window should produce no output and
        must not re-mark the briefing.
        """
        stale = [str(healthy_workspace / "src" / "stale.py")]
        with (
            patch.object(_user_prompt_submit, "stale_files", return_value=stale),
            patch.object(_user_prompt_submit, "briefing_due", return_value=False),
            patch.object(_user_prompt_submit, "mark_briefing_sent") as mock_mark,
        ):
            data = _run_hook(
                _user_prompt_submit,
                {"cwd": str(healthy_workspace)},
    ...
    def test_emits_staleness_warning_and_marks_sent(self, healthy_workspace, monkeypatch, capsys):
        """When healthy, stale, and the TTL has elapsed, the hook injects a
        staleness warning with repo-relative paths and marks the briefing
        sent so the next prompt in the same window stays quiet.
        """
        stale = [str(healthy_workspace / "src" / "stale.py")]
        with (
            patch.object(_user_prompt_submit, "stale_files", return_value=stale),
            patch.object(_user_prompt_submit, "briefing_due", return_value=True),
            patch.object(_user_prompt_submit, "mark_briefing_sent") as mock_mark,
        ):
            data = _run_hook(
                _user_prompt_submit,
        ...
        assert data is not None
        ctx = data["hookSpecificOutput"]["additionalContext"]
        assert "[OpenTrace] Graph staleness" in ctx
