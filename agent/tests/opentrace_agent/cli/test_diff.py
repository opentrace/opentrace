...
def test_run_diff_no_changes(tmp_path, capsys):
    ...
    with (
        ...
        patch("opentrace_agent.cli.diff.run_impact") as mock_impact,
    ):
...
def test_run_diff_invokes_impact_per_file(tmp_path, capsys):
    ...
    with (
        patch("subprocess.run", return_value=_completed(fake_out)),
        patch("opentrace_agent.cli.diff.run_impact") as mock_impact,
    ):
...
def test_run_diff_passes_staged_flag(tmp_path):
    ...
    with patch("subprocess.run", return_value=_completed("")) as mock_run, patch("opentrace_agent.cli.diff.run_impact"):
