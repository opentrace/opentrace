...
def main() -> None:
    ...
    if not env_flag_enabled("OPENTRACE_CLAUDE_AUTO_CONTEXT", default=AUTO_CONTEXT_DEFAULT):
        _debug("skip — auto context disabled")
        return

    tool_name = payload.get("tool_name", "") or ""
    tool_input = payload.get("tool_input", {}) or {}
    _debug(f"tool={tool_name!r} input_keys={list(tool_input)}")
    if tool_name == "Bash" and not env_flag_enabled("OPENTRACE_CLAUDE_AUGMENT_BASH", default=BASH_AUGMENT_DEFAULT):
        _debug("skip — bash augmentation disabled")
        return
    ...
    cache_key = context_cache_key(workspace_root, "PreToolUse", kind, value)
    if context_recently_emitted(cache_key):
        _debug("skip — duplicate context recently emitted")
        return
