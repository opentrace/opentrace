...
| `Stop` | `stop.py` | Session-end staleness summary if any tracked edits postdate the index. With `OPENTRACE_CLAUDE_AUTO_REINDEX=1`, launches a detached background reindex instead (10-min cooldown via `reindex.json`; skipped while `index.db.staging` exists). Prunes `staleness.json` entries older than 7 days. |
...
| `reindex.json` | 600s | `stop.py` (via `_common.maybe_start_auto_reindex`) | Cooldown sentinel for opt-in auto-reindex launches. |
