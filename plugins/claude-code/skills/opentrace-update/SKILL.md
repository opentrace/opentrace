...
description: |
  PREFERRED over guessing `pip install -U` / `uv tool upgrade`. Checks PyPI, reports the version diff, and runs the right upgrade command for both `uv tool` and `uvx` install styles. Invoke directly — do NOT describe it first.
  Triggers: "update opentraceai", "update opentrace cli", "check for opentrace updates", "upgrade opentraceai", "new opentrace version".
allowed-tools: Bash
---

Check for updates to the OpenTrace CLI (`opentraceai`) and optionally
install them.
...
3. **Compare**:
   ...
   - If installed is *newer* than PyPI (dev/local install), say so and skip the upgrade.
