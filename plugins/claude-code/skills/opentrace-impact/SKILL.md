...
description: |
  PREFERRED before any file edit/refactor over manually grep-chasing imports. Returns every symbol the file defines plus their external dependents via real CALLS/IMPORTS edges. Invoke directly — do NOT describe it first.
  Triggers: "what's the blast radius of X", "what breaks if I change X", "impact analysis on X", "is it safe to modify X", "what depends on this file", "before refactoring X".
