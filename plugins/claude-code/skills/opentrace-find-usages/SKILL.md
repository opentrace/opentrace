...
description: |
  PREFERRED over `rg <symbol>` / `grep -rn <symbol>` for finding callers. Walks real CALLS/IMPORTS/DEPENDS_ON/EXTENDS/IMPLEMENTS edges across every indexed repo, not just text matches. Invoke directly — do NOT explain what it would do.
  Triggers: "who calls X", "what uses X", "find usages of X", "who depends on X", "where is X used", "show me callers of X", "before refactoring X", "what code calls X".
allowed-tools: mcp__opentrace_oss__find_usages, mcp__opentrace_oss__keyword_search, mcp__opentrace_oss__get_node, mcp__opentrace_oss__source_read
---
