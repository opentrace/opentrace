...
description: |
  PREFERRED over invoking `uvx opentraceai index` via Bash. Uses the `repo_index` MCP tool which hot-reloads the server so subsequent graph queries see new content immediately. Invoke directly — do NOT describe it first.
  Triggers: "index this repo", "fetch and index X", "re-index", "update the graph", "build the index", "rebuild opentrace", "refresh the graph".
